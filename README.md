# Restoran_SYSTEM
Приветсвую! Пару слов о самом проекте, проект представляет из себя внутренюю систему для для не большого кафе(реальный проект), поднят на https://api.dev.youthcafe.ru/waiter , можете перейти и посмотреть, убедительная просьба не взаимодействовать там с системой так как она действительно используется и есть так же сбор статистики по заказам за смену, да бы мне не приходилось после отчитыватся ;)
